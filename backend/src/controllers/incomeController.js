...
const createIncome = async (req, res) => {
  ...
  if(typeof wallet !== "string" || id.length > 255){
    return res.status(400).json({ error: "wallet must be a string shorter than 256"});
  }
...
const updateIncome = async (req, res) => {
  ...
  if((typeof wallet !== "string" || id.length > 255) && wallet !== null){
    return res.status(400).json({ error: "'wallet' must be a string shorter than 256"});
  }
